...
```bash
$ echo "CID=$(cat ~/.config/plasma-org.kde.plasma.desktop-appletsrc | grep "Applets" | head -n 1 | awk -F'[][]' '{print $4}')"  
# CID=2 for me
```
...
```bash
$ busctl --user call org.kde.plasmashell /PlasmaShell org.kde.PlasmaShell evaluateScript 's' "p=panelById(2); print(JSON.stringify(p, null, 2))" | sed -n 's/^s "\(.*\)"$/\1/p' | sed 's/\\n/\n/g' | sed 's/\\"/"/g' | jq
```
